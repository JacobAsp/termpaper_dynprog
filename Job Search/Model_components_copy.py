...
def SolveModel(params, institutions, abar, htm):
    ...
    if htm == 1:
        ...
        a_path = np.zeros((n_a_eff, T+1))  # placeholder variable for assets in period t with asset value a.

        for t in np.arange(T-1, -1, -1):
            # Current unemployment income
            y = benefits[t]

            # consumption possibilites - det her er der vel ingen grund til at vi definerer inden i loopet?
            ...
            for i in range(n_a_eff):        # vi kigger på alle rækkerne og tilføjer forbrugsmuligheder fra 0 og op til maks forbrug som er givet ved assets + løn.  
                c[i, :] = np.linspace(abar_local[0], (a[i,0]+y), n_c_eff)     # vi går fra 0 til det højeste asset level i den række plus overførselsindkomsten

            # a for assets i næste periode skal afhænge af, hvor meget du forbrugte i seneste periode. 

            a1 = (a - c + y) * (1 + R)   # assets i næste periode   a1 er en na x nc matrix. 
            ...
            a_path[:,t] = a1[np.arange(n_a_eff), i_max]                # her definerer vi at 
    ...
    return S, V_emp, V_uemp, c_emp, c_uemp, Vss_emp, Vss_uemp, css_emp, css_uemp, survival, benefits, a_path
