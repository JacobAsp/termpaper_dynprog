...
def steady_state_Vu_fsolve(Ve, k, gamma, delta, benefits):
    '''
    ...
    x0 = Ve
    # print(steady_state_FOC(x0, V_E[T-1],k,gamma,delta,benefits))
...
def solveSingleTypeModel(params,institutions):
    ...
    T,w_pre,w_post,benefits, R, A_grid  = institutions

    # # Try to avoid zero division errors
    # delta = min(delta,1-1e-10)
    ...
    nA = len(A_grid)
    # replace search and value vectors with matrices
    s = np.zeros((T,nA))
    V_E = np.zeros((T,nA))
    V_U = np.zeros((T,nA))
    # A policy stores the structural changes in the asset grid over time.
    #That is A*_t+1(A_i)
    # what is the optimal asset level in next period, given current asset level A_i in period t
    A_policy = np.zeros((T,nA))
    # Now V_U[t,i] means V_t^U[A_i], value of being unemployed in period t with asset level A_i
    #s[t,i] means s_t[A_i], search effort in period t with asset level A_i
    ...
    for t in np.arange(T-int(np.ceil(N))):
        for a, A in enumerate(A_grid):
                cE = w_post + R/(1+R) * A
                V_E[T-1, a] = v(cE) / (1-delta)
                #V_E[t] = v(w_post) / (1-delta)
                V_E[t] = v(cE) / (1-delta)
                # Add gain-loss part
                for i in np.arange(int(np.ceil(N))):
                    V_E[t,a] = V_E[t] + eta * delta**i * (v(w_post) - v(r[t,t+i])  )
    ...
    for a, A in enumerate(A_grid):
        V_U[T-1, a] = steady_state_Vu(V_E[T-1, a], k, gamma, delta, benefits)
        # print(V_U[T-1, a])
        # print(steady_state_FOC(V_U[T-1, a], V_E[T-1, a],k,gamma,delta,benefits))
    ...
    for t in np.arange(T-2,-1,-1):
        ...
        for i, A_t in enumerate(A_grid):
            ...
            # candidate values for each possible next period asset level
            values = np.full(A_grid.shape[0], -np.inf)

            for j, A_next in enumerate(A_grid):
                c_t = A_t + IncomePaths[T,t] - A_next/(1+R)

                if c_t <= 0:
                    continue
                
                s_candidate = cost_prime_inverse(V_U[t+1,j], V_E[t+1,j], k,gamma,delta)
                # Bellman equation
                values[j] = u(c_t,r[T,t],eta,lam)  - cost(s_candidate,k,gamma)+delta* (s_candidate* V_E[t+1, j] + (1-s_candidate)*V_U[t+1, j])
            # Pick optimal  next period consumption
            j_star = np.argmax(values)
            V_U[t,i] = values[j_star]
            A_policy[t,i] = A_grid[j_star]

            s[t,i] = cost_prime_inverse(V_U[t+1,j_star], V_E[t+1,j_star], k,gamma,delta)
            # print(t)


            #s[t] = cost_prime_inverse(V_U[t+1], V_E[t+1], k,gamma,delta)
            # print(IncomePaths[T,t])
            #c_t = IncomePaths[T,t] + R/(1+R) * A_grid
            #V_U[t] = u(c_t,r[T,t],eta,lam) \
